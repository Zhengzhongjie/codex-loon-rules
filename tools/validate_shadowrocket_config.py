#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate the Shadowrocket config — dual mode.

Without an argument (CI mode): validate the committed skeletons in configs/shadowrocket/ — they must
match the generator output exactly (no manual drift) and carry only placeholders in private sections —
plus the generated Shadowrocket rule tree. With a config path (local mode): validate a filled private
`.conf` — placeholders gone, private sections populated — or a skeleton path, auto-detected by the
presence of `{{…}}`. Either way the [Rule] bindings must still match the RULESETS catalogue, so the
config can never drift from the generated rule trees.
"""
...
CONFIGS_DIR = REPO_ROOT / "configs" / "shadowrocket"
SR_RULE_TREE = REPO_ROOT / "rules" / "shadowrocket" / "generated"
...
def check_structure(text: str, name: str) -> list[str]:
    ...
    expected_rule_sets = [ln for ln in sr.rule_section() if ln.startswith("RULE-SET,")]
    actual = secs["Rule"]
...
def validate_committed_skeletons() -> list[str]:
    ...
    for device in sr.DEVICES:
        ...
        if not path.exists():
            ...
            continue
        text = path.read_text()
        if text != sr.render_config(device):
            errors.append(f"{path.name}: skeleton drifted from generator — rerun build_shadowrocket_config.py")
...
def main() -> int:
    ...
    if args.config is None:
        ...
        errors += validate_generated_tree(SR_RULE_TREE)
    else:
    ...
    print("OK: Shadowrocket config invariants passed")
    return 0
